...
def hf_transcript(audio_file_path: str, model: str):
    ...
    try:
        ...
        raise ValueError(
            f"Could not access model '{model}' via Hugging Face API. It may be invalid or unavailable.") from e
    except Exception as e:
        logger.error(f"Error processing audio file '{audio_file_path}' with Hugging Face API: {e}")
        raise IOError(
            f"Failed to process audio file with HF API. It may be corrupt or in an unsupported format.") from e

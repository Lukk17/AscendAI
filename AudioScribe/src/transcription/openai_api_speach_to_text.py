...
# Initialize the client with the timeout from settings
client = OpenAI(timeout=httpx.Timeout(settings.API_TIMEOUT_SECONDS))

def _transcribe_single_chunk(audio_chunk_path: str, model: str) -> str:
...
def openai_transcript(audio_file_path: str, model: str = "whisper-1"):
    ...
    if file_size < settings.OPENAI_API_LIMIT_BYTES:
        ...
        return _transcribe_single_chunk(audio_file_path, model)
    ...
    ratio = file_size / len(audio)  # bytes per millisecond
    ...
    try:
        ...
        for i, start_ms in enumerate(range(0, len(audio), chunk_length_ms)):
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_audio:
                ...
                chunk_transcription = _transcribe_single_chunk(tmp_audio.name, model)

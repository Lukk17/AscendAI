...
- `OCR_SCRATCH_DIR` — directory for the worker's temporary upload copy; swept of anything older than `OCR_REQUEST_TIMEOUT + OCR_DISPATCH_MARGIN_SECONDS` by every fresh worker and at startup, so a killed worker's leaked file cannot accumulate (default a `paddle-ocr-scratch` directory under the system temp path).

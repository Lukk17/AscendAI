...
| `rest_endpoints.py` | `src/api/rest/rest_endpoints.py` | `APIRouter(prefix="/v1")`. Validates file type and size, reads upload bytes, dispatches to the OCR worker process via `loop.run_in_executor(get_process_pool(), ...)`. |
| `mcp_server.py` | `src/api/mcp/mcp_server.py` | FastMCP instance. Owns the `aiohttp.ClientSession` lifespan. `ocr_process` tool: scheme dispatch, SSRF guard, `file://` jail, size enforcement, OCR worker process call. |

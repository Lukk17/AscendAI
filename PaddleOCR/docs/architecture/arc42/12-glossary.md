...
| **S3-compatible object store** | The document store used across the monorepo stack, reached over the S3 REST API. PaddleOCR's MCP tool fetches files from it when `file_uri` is an `http://host.docker.internal:9070/...` URL. Requires `MCP_ALLOWED_HOSTS=host.docker.internal` to bypass the SSRF IP block. Locally provided by a self-hosted emulator; an external prerequisite, not a compose service. |

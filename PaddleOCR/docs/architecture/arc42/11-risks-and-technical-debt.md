...
| **Single-worker Uvicorn** | Low | Medium | The default compose `CMD` starts one Uvicorn worker. Long OCR jobs (large PDFs) queue all concurrent requests behind the single OCR worker process, since `_WORKER_POOL_SIZE` is fixed at 1. Adding `--workers N` requires testing for session isolation of the module-level `_http_session` in `mcp_server.py`, which is a global. |

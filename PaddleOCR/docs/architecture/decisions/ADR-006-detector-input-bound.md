...
- `PaddleOCR/src/service/ocr_service.py` — `OcrService._get_engine`.
- `PaddleOCR/src/config/config.py` — `OCR_DETECTOR_MAX_SIDE`, `OCR_MAX_INFERENCE_PIXELS`.
- `PaddleOCR/src/api/limits.py` — the pixel ceiling enforced at the request boundary, before any worker is touched.

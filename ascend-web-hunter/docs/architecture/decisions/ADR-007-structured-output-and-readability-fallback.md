...
1. **No article metadata.** Title, author, publication date, and site name are available from trafilatura when
   called in JSON mode but are discarded. Callers (AscendAgent RAG pipeline) benefit from structured metadata
...
- **Cons**: All existing REST and MCP callers expect the flat shape. A breaking API change would require
  updating AscendAgent and any integration tests at the same time as this slice.
...
### Positive
- AscendAgent RAG and display layers gain access to article metadata without a separate extraction pass.

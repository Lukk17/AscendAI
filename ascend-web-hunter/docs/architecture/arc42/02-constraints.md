...
| In-monorepo service | `ascend-web-hunter/` directory inside `AscendAI` | Shares AGENTS.md conventions, docker-compose network, and the `ascend-scrapper.docker-compose.yaml` external services group. Cannot be extracted without carrying those dependencies. |

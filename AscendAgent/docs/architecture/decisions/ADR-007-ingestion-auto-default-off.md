...
- **No surprise embedding spend.** The auto-poller picks up *every* file that lands in the bucket. With paid embedding APIs (OpenAI, Gemini paid tier), an operator who drops a 500-page corpus into the object store during exploration would burn a noticeable amount of credit before realizing it. Manual `/run` puts that decision in the operator's hands.
...
- **Forgotten ingestion.** A new operator who drops a file into the object store and waits will see nothing happen until they call `/run` or read the docs. Mitigated by surfacing the manual flow as the primary path in `docs/INGESTION.md` and making the curl examples obvious in the testing docs.

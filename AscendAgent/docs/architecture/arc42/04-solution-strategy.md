...
| :-------------------------------- | :---------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------- |
| Multi-provider AI                 | `ChatModelResolver` manages a `Map<String, ChatModel>`. Per-request resolution via the `provider` param.                | [ADR-001](../decisions/ADR-001-multi-provider-ai.md)                                 |
...
- **Strategy pattern.** `ChatModelResolver` selects the `ChatModel` implementation based on provider name.
- **Builder pattern.** `ChatClient` built per request with resolved model, system prompt, MCP tools, and optional
  model override.
- **Pipeline pattern.** Document ingestion flows S3 to Unstructured API to Token splitter to Vector store.

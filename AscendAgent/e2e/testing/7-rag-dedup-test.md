...
Hermetic: owns the two `dedup-pierogi-*.md` fixtures in MinIO + Qdrant + the `dedup-test` chat-history rows.
...
Truncate `dedup-test` chat-history.

```bash
docker exec postgres psql -U postgres -d ascend_ai -c "DELETE FROM chat_history WHERE user_id = 'dedup-test';"
```

```bash
docker exec redis redis-cli DEL chat:dedup-test
```

...
class ChatExecutorBranchCoverageTest {
    ...
    void execute_WhenCacheConfigError_RetriesWithoutCache() {
        ...
        PromptCacheStrategy alwaysCacheError = new PromptCacheStrategy() {
            ...
            @Override
            public org.springframework.ai.chat.prompt.ChatOptions buildOptions(String model) {
                return null;
            }

            @Override
            public void recordOutcome(String userId, org.springframework.ai.chat.model.ChatResponse response) {
            }
    ...
    private ChatResponse buildResponse() {
        ...
        when(msg.getText()).thenReturn(TestConstants.TEST_RESPONSE_CONTENT);
    ...
    private static PromptCacheStrategy anthropicStrategyWithCacheOptions() {
        org.springframework.ai.anthropic.api.AnthropicCacheOptions cacheOpts =
        ...
        org.springframework.ai.anthropic.AnthropicChatOptions opts =
                org.springframework.ai.anthropic.AnthropicChatOptions.builder().cacheOptions(cacheOpts).build();

        return new PromptCacheStrategy() {
            @Override
            public String providerName() {
                return "anthropic";
            }

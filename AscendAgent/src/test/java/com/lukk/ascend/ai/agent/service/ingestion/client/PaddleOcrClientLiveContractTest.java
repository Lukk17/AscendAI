...
class PaddleOcrClientLiveContractTest {

    private static final String BASE_URL = "http://localhost:7022";
    private static final String API_PATH = "/v1/ocr";

    // Captured live from a running PaddleOCR instance so these assertions are pinned to what the
    // service actually publishes, not to a guess. Regenerate with:
    //   curl http://localhost:7022/openapi.json > src/test/resources/paddleocr/openapi-contract.json
    //   curl -F "file=@PaddleOCR/e2e/fixtures/argent-saga-chronicles-page1-polish.png" \
    //        http://localhost:7022/v1/ocr > src/test/resources/paddleocr/real-ocr-response.json
    private static final String OPENAPI_CONTRACT_RESOURCE = "/paddleocr/openapi-contract.json";
    private static final String REAL_OCR_RESPONSE_RESOURCE = "/paddleocr/real-ocr-response.json";

    private final ObjectMapper objectMapper = new ObjectMapper();

    @Test
    @DisplayName("process sends the multipart part name and query param name PaddleOCR's own OpenAPI contract requires")
    void process_SendsPartAndQueryParamNamesFromRealContract() throws IOException {
        ...
        PaddleOcrClient client = new PaddleOcrClient(builder.build(), objectMapper, BASE_URL, API_PATH);

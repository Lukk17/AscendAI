...
- **`GET /api/v1/documents/{id}/content`** — authenticated, ownership-scoped download that streams the document's MinIO object bytes back through the agent. This becomes the client-facing download path for RAG source attachments: `AiResponse.sources[*]` (synchronous) and the streaming `sources` event carry the registry document id and this relative content path instead of a presigned MinIO URL. Presigned URLs are demoted to an in-network/optional mechanism, so MinIO never needs to be publicly exposed (coordinated with `harden-cloud-deployment`'s gateway-only surface, `add-chat-streaming-and-conversations`'s `sources` event, and `add-tenant-isolation`'s ownership check).
...
- `rag-source-attachments`: source attachments carry the registry document id and the relative `/api/v1/documents/{id}/content` path; the client downloads through the authenticated agent endpoint rather than a presigned MinIO URL. Presigning is retained only as an in-network mechanism. (This is the download-mechanism half of the presign-resolution amendment; `add-tenant-isolation` layers the per-tenant ownership check onto the same endpoint.)

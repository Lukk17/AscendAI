...
- [Breaking `/run` response contract] Existing Bruno requests and the e2e specs assert on synchronous counts. → Mitigation: update the Bruno collection and `apps/ascend-ai-agent/e2e/` specs in the same change; version stays `/api/v1` because the endpoint's consumers are all in-repo.

...
- [ ] 7.1 Update `apps/ascend-ai-agent/AGENTS.md`: document the new `/api/v1/documents` endpoints, the async `/api/v1/ingestion/run` contract, and the registry tables
...
- [ ] 7.3 Update `apps/ascend-ai-agent/e2e/` specs that assert on the synchronous `/run` response (poll `GET /api/v1/ingestion/runs/{id}` instead) and note the change in `e2e/README.md` if the capability matrix mentions run semantics

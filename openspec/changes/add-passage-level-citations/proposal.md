...
- Page provenance survives ingestion on every parser path. Unstructured elements are grouped by `metadata.page_number` into one document per page, PaddleOCR pages become one document per `page_number`, and `DocumentRouter` stamps the page number it already computes as structured metadata instead of burying it in a synthetic filename. The original object key stays the chunk's `source` on the per-page PDF path.
...
- Affected code (AscendAgent):
  ...
  - `service/ingestion/IngestionService.java`, `service/ingestion/client/PaddleOcrClient.java`, `service/ingestion/client/DoclingClient.java`, `service/ingestion/DocumentRouter.java`, `service/ingestion/DocumentService.java`, `service/ingestion/ManualIngestionService.java`, `service/ingestion/IngestionMetadataKeys.java`: page and ordinal provenance.
- Configuration: the `app.system-prompt` Citations block is rewritten in `AscendAgent/src/main/resources/application.yaml`, and new `app.rag.citations.*` keys are bound through `config/properties/RagProperties.java`.
...
- Architecture documentation: two new records under `AscendAgent/docs/architecture/decisions/`, numbered ADR-010 and ADR-011.

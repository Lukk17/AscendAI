...
- Configuration: the `app.system-prompt` Citations block is rewritten in `apps/ascend-ai-agent/src/main/resources/application.yaml`, and new `app.rag.citations.*` keys are bound through `config/properties/RagProperties.java`.
...
- Architecture documentation: two new records under `apps/ascend-ai-agent/docs/architecture/decisions/`, numbered ADR-010 and ADR-011.

...
- [ ] 7.4 AscendWebSearch: the same REST dependency and FastMCP enforcement, `/health` open
...
## 9. AscendAgent outbound token attachment
...
- [ ] 9.3 Attach the header on all three MCP client connections (audioscribe, weather, ascend-web-search), through Spring AI connection header configuration, falling back to a WebClient customizer where the connection type lacks header support
...
- [ ] 11.2 Update the five e2e specs and their tasks-templates in `AscendAgent/e2e/` for the secured posture, adding the token step to setup and removing `X-User-Id` usage, while keeping the dev-profile path documented for ad-hoc manual runs
...
- [ ] 12.7 Update the root `AGENTS.md` and the per-module `AGENTS.md` files (AscendAgent, AscendMemory, AudioScribe, AscendWebSearch, PaddleOCR, WeatherMCP) with the new environment variables, the secured-by-default posture, and the dev-profile note
...
- [ ] 12.9 Update `AscendAgent/docs/architecture/` diagrams and arc42 sections for the resource-server posture and the principal resolution path

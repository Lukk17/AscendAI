...
The URL frontier lives in Redis: a per-domain queue with crawl-delay and a per-domain concurrency cap, seeded from sitemaps when present, honouring robots.txt (toggleable per job for internal sites the customer owns). Scale is adding AscendWebSearch worker containers that consume the same frontier — no central coordinator beyond Redis. Politeness is a first-class default, not an afterthought, because an impolite crawler gets the deployment's IP banned.

...
- [ ] 4.2 Engage the proxy only for crawl jobs when configured; document that AscendWebSearch never runs or resells proxies
...
- [ ] 5.2 On each scheduled run, trigger an AscendWebSearch crawl, land the output in the tenant's MinIO prefix, and trigger the existing ingestion pipeline (land-bytes-then-ingest, no parallel parse path)
...
- [ ] 6.1 Update `AscendWebSearch/AGENTS.md`: crawl API, frontier politeness, incremental recrawl, BYO-proxy hook, and an honest scale statement (hundreds-of-thousands-of-pages class, not proxy-network scale)
- [ ] 6.2 Extend `docs/CONNECTORS.md` (from `add-document-connectors`) with the web connector setup
- [ ] 6.3 Run `pytest` for AscendWebSearch and `./gradlew test` for AscendAgent's connector additions; all green

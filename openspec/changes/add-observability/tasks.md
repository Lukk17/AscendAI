...
- [x] 2.1 Add `org.springframework.boot:spring-boot-starter-actuator` and `io.micrometer:micrometer-registry-prometheus` to `AscendAgent/build.gradle.kts`
...
- [x] 10.1 Add `prometheus-fastapi-instrumentator`, `opentelemetry-distro`, `opentelemetry-exporter-otlp` to `AscendMemory/pyproject.toml`
- [x] 10.2 In `AscendMemory/src/main.py`, after FastAPI app construction: `Instrumentator().instrument(app).expose(app)` for `/metrics`

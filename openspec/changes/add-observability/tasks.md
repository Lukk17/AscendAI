...
- [x] 7.2 Mirror `application.yaml` management block from ascend-ai-agent with `service: ascend-weather-mcp`
...
- [ ] 15.1 Confirm `/actuator/env`, `/actuator/heapdump`, `/actuator/threaddump`, `/actuator/loggers`, `/actuator/caches` return 404 by default on ascend-ai-agent and ascend-weather-mcp
...
- [ ] 15.4 Add an integration test `OtelTraceShipsToTempoIT` that sends one chat prompt via ascend-ai-agent, polls Tempo for a trace with `service.name=ascend-ai-agent`, asserts the trace has at least one LLM-call span

...
- One inference worker. `_WORKER_POOL_SIZE = 1` in
  [ocr_service.py](../../../apps/ascend-ocr/src/service/ocr_service.py), a `ProcessPoolExecutor` on a `spawn` context.
...
- The timeout is `asyncio.wait_for(loop.run_in_executor(...), timeout=settings.OCR_REQUEST_TIMEOUT)` in both
  ...
  [mcp_server.py](../../../apps/ascend-ocr/src/api/mcp/mcp_server.py). It cancels the awaiting coroutine and nothing else.
...
`apps/ascend-ocr/AGENTS.md`, the README and the arc42 pages beside the setting, so raising the worker count reads as a
...
page rather than on the scanner. It lands as an ADR under `apps/ascend-ocr/docs/architecture/decisions/` in the format of

...
- [x] 2.4 First job `changes`: `dorny/paths-filter@v3` with one filter per service (`ascend-agent`, `weather-mcp`, `audio-scribe`, `ascend-web-hunter`, `ascend-memory`, `ascend-paddle-ocr`) plus a `workflows` filter (`.github/workflows/**`) that forces all services to run

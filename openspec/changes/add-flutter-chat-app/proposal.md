...
- Implement an API client layer that connects to the AscendAgent REST endpoints (`POST /api/v1/ai/prompt`, `POST /api/v1/ai/prompt/stream` via server-sent events, and the `/api/v1/conversations` family).
...
### New Capabilities
- `flutter-chat/core-chat`: The primary chat screen powered by `flutter_chat_ui`, including the `Chat` widget integration, custom `ChatController` backed by AscendAgent conversations API, real-time server-sent events streaming for assistant responses, message sending with text, and display of system messages.
...
- Depends on the AscendAgent HTTP API (port 9917), specifically the existing `POST /api/v1/ai/prompt` and the specced streaming and conversation endpoints from the `add-chat-streaming-and-conversations` change.

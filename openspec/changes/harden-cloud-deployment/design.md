...
- [Real `X-Forwarded-For` may trip SearXNG's limiter for the agent's own high-volume search traffic] → the limiter distinguishes clients; the agent is one client. If throttling appears, the limiter's bot-detection thresholds are tunable in `searxng/settings.yml` — an operational knob, not a design flaw.

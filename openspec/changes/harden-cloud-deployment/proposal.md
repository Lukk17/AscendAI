...
- **`AscendAgent/src/main/resources/application.yaml` + `application-docker.yaml`**: object-store / Postgres / Redis / Qdrant credentials become env-parameterized with dev-only fallbacks; production posture rejects fallback values at startup.

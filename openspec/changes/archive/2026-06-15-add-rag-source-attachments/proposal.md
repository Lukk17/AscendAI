...
- **Size cap**: the property `app.rag.source-attachments.max-file-size` — defined in `application.yaml` (shipped default `1 GB`; the `RagProperties` code-level fallback is `25 MB`) — controls which retrieved sources are eligible. Sources larger than the cap are skipped with a single WARN log per request and do not appear in the response array; the rest are still returned.

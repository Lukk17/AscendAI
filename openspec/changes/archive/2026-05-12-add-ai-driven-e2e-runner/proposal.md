> **SUPERSEDED** — the goal of this proposal (AI-runnable e2e suite, per-run timestamped run records, captured verdicts) was met by the alternative e2e/testing reorganization shipped in commits `c791cc5` and `2edcbee`. That implementation uses a simpler architecture: explicit behavior assertions inside each `<N>-<feature>-test.md` spec (HTTP status / response body / persisted state) judged directly by the runner subagent, instead of a separate tracked `judge-prompt.md` with PASS/FAIL/INCONCLUSIVE rubric, temperature-0 retry, and cost cap. The formalisms below (judge prompt, INCONCLUSIVE bucket, retry-on-disagreement, cost-cap fuse) were considered and dropped — across three full sweeps the simpler in-spec assertions produced stable PASS verdicts without the extra layer. Archived as historical artifact of what was considered.

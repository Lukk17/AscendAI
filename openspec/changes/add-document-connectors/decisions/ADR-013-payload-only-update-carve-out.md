...
The native Qdrant client is already a declared dependency as `libs.qdrant.client` in `AscendAgent/build.gradle.kts`, so this is a second and narrower use of something already present, not a new dependency.

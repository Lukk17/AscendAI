...
ascend-ai-agent ships as a reproducible container image and runs as a service in the monorepo compose stack, so a single compose bring-up starts the whole platform including the gateway rather than leaving the gateway to be started by hand on the host. The capability covers the multi-stage build and its JRE-only non-root runtime, the container healthcheck, the compose service entry, container-mode parity through the `docker` Spring profile, provider API keys arriving as runtime environment from a developer-local `.env` instead of being baked into the image, reaching host-only prerequisites over `host.docker.internal`, and the pruned build context.

## Requirements
### Requirement: ascend-ai-agent ships a multi-stage Dockerfile

`apps/ascend-ai-agent/Dockerfile` SHALL define a two-stage build. The `builder` stage SHALL use `eclipse-temurin:21-jdk-alpine` and produce a Spring Boot fat jar via `./gradlew --no-daemon bootJar -x test`. The runtime stage SHALL use `eclipse-temurin:21-jre-alpine`, copy the bootJar from the builder, run as a non-root user, expose port `9917`, and use `ENTRYPOINT ["java","-jar","/app/app.jar"]` in exec form so Linux signals propagate to the JVM.

#### Scenario: Image builds successfully

- **WHEN** a developer runs `docker build -t ascend-ai-agent:dev ./apps/ascend-ai-agent` against a clean checkout
...
- **WHEN** the image is started with `docker run --rm ascend-ai-agent:dev id`
...
- **WHEN** a developer edits a Java source file under `apps/ascend-ai-agent/src/main/java/` and rebuilds the image
...
- **THEN** within 90 seconds `docker inspect --format '{{.State.Health.Status}}' ascend-ai-agent` returns `healthy`
...
The `ascend-ai-agent` compose entry SHALL set `SPRING_PROFILES_ACTIVE=docker` to activate `apps/ascend-ai-agent/src/main/resources/application-docker.yaml`. That profile SHALL override every URL and host that defaults to `localhost` in `application.yaml` so ascend-ai-agent works correctly from inside the container without any further env-var configuration by the operator. The profile SHALL cover at minimum:
...
- **WHEN** `ascend-ai-agent` starts with `SPRING_PROFILES_ACTIVE=docker`
...
- **THEN** `docker exec ascend-ai-agent env | grep OPENAI_API_KEY` shows the runtime value the developer set in `.env`

#### Scenario: Built image carries no secrets

- **WHEN** a reviewer runs `docker history ascend-ai-agent:latest` and `docker run --rm ascend-ai-agent:latest env`
...
- **THEN** ascend-ai-agent forwards the request to the host LM Studio successfully and returns a 200 response
### Requirement: Build context is pruned via `.dockerignore`

`apps/ascend-ai-agent/.dockerignore` SHALL exclude build artifacts (`build/`, `.gradle/`), IDE files (`.idea/`, `.vscode/`, `*.iml`), logs, OS metadata, documentation (`docs/`, `e2e/`, `README.md`, `AGENTS.md`), and any local secret files (`.env`, `.env.local`, `.env.*.local`) from the Docker build context, so the image is small, builds are fast, and host-only artifacts cannot leak into image layers.

#### Scenario: Build context excludes host artifacts

- **WHEN** the Docker daemon prepares the build context for `docker build ./apps/ascend-ai-agent`
- **THEN** the context tarball does NOT contain `apps/ascend-ai-agent/build/`, `apps/ascend-ai-agent/.gradle/`, any `.iml` file, any `.idea/` directory, or any `.env*` file

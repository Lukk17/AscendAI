...
AscendAgent ships as a reproducible container image and runs as a service in the monorepo compose stack, so a single compose bring-up starts the whole platform including the gateway rather than leaving the gateway to be started by hand on the host. The capability covers the multi-stage build and its JRE-only non-root runtime, the container healthcheck, the compose service entry, container-mode parity through the `docker` Spring profile, provider API keys arriving as runtime environment from a developer-local `.env` instead of being baked into the image, reaching host-only prerequisites over `host.docker.internal`, and the pruned build context.

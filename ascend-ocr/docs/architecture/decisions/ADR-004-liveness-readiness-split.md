...
- `ascend-ocr/src/main.py` — `health_check`, `readiness_check`.
- `ascend-ocr/src/model/ocr_models.py` — `HealthResponse`, `ReadinessResponse` (`accepting_work`, `queue_depth`).
- `ascend-ocr/src/observability/metrics.py` — `is_engine_warm`, the multiprocess-file readiness check.
- `ascend-ocr/src/service/ocr_service.py` — `start_worker_pool`, `_warm_worker_engine`, `is_accepting_work`,
  `is_pool_usable`, `is_rebuild_in_progress`, `is_job_overrunning`, `_active_deadlines`, `_rebuild_pool`, the
  `BrokenProcessPool` handling.
- `ascend-ocr/src/config/startup_banner.py` — emits both URLs and the per-worker memory arithmetic at startup.
- `ascend-ocr/tests/api/rest/test_rest_endpoints.py` — `TestReadyEndpoint`, `test_unexpected_dispatch_exception_returns_500_not_a_crash`.
- `ascend-ocr/tests/service/test_ocr_service.py` — `TestPoolHealthSignals`, `TestRebuildPool`,
...
- `ascend-ocr/tests/observability/test_metrics.py` — `TestIsEngineWarm`.
- `ascend-ocr/Dockerfile` — `HEALTHCHECK` points at `/health`, not `/ready`.

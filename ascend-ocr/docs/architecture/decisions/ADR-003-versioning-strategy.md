...
- `ascend-ocr/src/api/rest/rest_endpoints.py` — `APIRouter(prefix="/v1")`.
- `ascend-ocr/src/api/mcp/mcp_server.py` — `@mcp.tool() async def ocr_process(...)`.
- `ascend-ocr/src/model/ocr_models.py` — `OcrJsonResponse.schema_version: Literal["1"] = "1"`.

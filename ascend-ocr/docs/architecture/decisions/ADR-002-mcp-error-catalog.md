...
- `ascend-ocr/src/api/exception_handlers.py` — class definitions, handlers, code constants.
- `ascend-ocr/src/api/mcp/mcp_server.py` — raises the same exception classes from the MCP path.
- `ascend-ocr/src/api/rest/rest_endpoints.py` — raises the same exception classes from the REST path.
- `ascend-ocr/tests/api/test_exception_handlers.py` — asserts shape and that no internal detail leaks.

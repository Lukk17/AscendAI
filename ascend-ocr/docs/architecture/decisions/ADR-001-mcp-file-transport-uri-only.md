...
- `ascend-ocr/src/api/mcp/mcp_server.py` — `ocr_process`, `_fetch_file`, `_read_jailed_file`, `_download_http`, `_validate_host`, `_is_blocked`, `_is_within`.
- `ascend-ocr/src/config/config.py` — `MCP_FILE_URI_ROOT`, `MCP_ALLOWED_HOSTS`, `MCP_DOWNLOAD_TIMEOUT_SECONDS`.
- `ascend-ocr/e2e/testing/6-mcp-ocr-test.md` — operator-facing test that exercises this path; requires `host.docker.internal` in `MCP_ALLOWED_HOSTS`.

...
The network alias `ascend-ocr` (and the hostname `ascend-ocr`) allows the AscendAgent to reach the
...
(`ascend-ocr/Dockerfile:58` sets no explicit `HEALTHCHECK` instruction — the base image default applies, or the
...
The Dockerfile uses a two-stage build (`ascend-ocr/Dockerfile`). The builder stage installs dependencies and pre-caches

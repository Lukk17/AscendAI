...
```powershell
Test-Path ascend-audio-scribe/e2e/fixtures/meeting-clip.wav
```
...
```powershell
curl.exe -sS -o NUL -w "%{http_code}\n" -X PUT -H "Content-Type: audio/mpeg" --data-binary "@ascend-audio-scribe/e2e/fixtures/meeting-clip.wav" "http://localhost:9070/e2e-fixtures/meeting-clip.wav"
```
...
- `ascend-audio-scribe/e2e/fixtures/meeting-clip.wav` — same fixture used by spec `2-transcribe-openai-test.md`. The MCP

...
The Orchestrator originally used a single `ChatClient` bean wired to LM Studio. To support multiple AI providers (OpenAI, Gemini, Anthropic, MiniMax) without restarting the application, we need a mechanism for per-request provider and model selection.

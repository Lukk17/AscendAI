...
`HF_TOKEN` on the ascend-audio-scribe container and outbound HTTPS to `api.openai.com` / `api-inference.huggingface.co`. `4`
...
flowchart LR
    ...
    Req -->|HTTP POST multipart or /mcp| Scribe[ascend-audio-scribe :7017]
...
6. **Fixtures.** Paths under `apps/ascend-audio-scribe/e2e/fixtures/` to the canary audio files the test uploads.
...
1. Docker compose stack up: `docker compose up -d --build ascend-audio-scribe` (or include in the full `ascend-ai` stack).
2. `curl -fsS http://localhost:7017/health` returns HTTP 200 with `{"status":"ok","service":"ascend-audio-scribe"}`.
3. Bruno CLI installed: `bru --version` returns a version string. Install once with `npm install -g @usebruno/cli`.
4. For tests 2 and 5: `docker exec ascend-audio-scribe sh -c '[ -n "$OPENAI_API_KEY" ] && echo present || echo missing'` prints

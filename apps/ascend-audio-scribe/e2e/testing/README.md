...
Self-contained walkthroughs that an AI agent (or a human) can execute end-to-end against a live ascend-audio-scribe container.
Each `<N>-<capability>-test.md` spec in this directory drives one ascend-audio-scribe capability through its Bruno request,
...
and aren't visible from every runner's shell. If a behaviour assertion fails, a tail of the ascend-audio-scribe log (or
`docker logs ascend-audio-scribe`) is the next diagnostic step, but not a pass criterion.
...
2. Create `apps/ascend-audio-scribe/e2e/testing/<N>-<capability>-test.md` using the template above. Pick the lowest unused number
   prefix that matches its setup-cost position in the order.
3. Create `apps/ascend-audio-scribe/e2e/testing/templates/<N>-<capability>-tasks.template.md` mirroring the spec's checkboxes.

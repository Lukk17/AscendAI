...
A drop-in skill ships at [skills/ascend-audio-scribe/SKILL.md](skills/ascend-audio-scribe/SKILL.md). Copy
[skills/ascend-audio-scribe/](skills/ascend-audio-scribe/) into your agent's skills folder (`.claude/skills/`, `.agents/skills/`,
...
**1. Install dependencies.** From the `apps/ascend-audio-scribe/` directory:
...
```bash
docker logs --tail 80 -f ascend-audio-scribe
```

Force a clean recreate after a Dockerfile or env-var change:

```bash
docker compose up -d --build --force-recreate ascend-audio-scribe
```
...
```bash
docker run -d --name ascend-audio-scribe --gpus all -p 7017:7017 -e OPENAI_API_KEY="sk-..." -e HF_TOKEN="hf_..." -e HF_HOME=/hf-cache -v ~/hf-cache:/hf-cache ascend-audio-scribe:latest
```

PowerShell:

```powershell
docker run -d --name ascend-audio-scribe --gpus all -p 7017:7017 -e OPENAI_API_KEY="sk-..." -e HF_TOKEN="hf_..." -e HF_HOME=/hf-cache -v "D:/path/to/your/hf-cache:/hf-cache" ascend-audio-scribe:latest
```

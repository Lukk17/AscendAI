...
```bash
bru run "memory/testing/mcp-insert.yml" --env ascend-local --env-var "mcp_session_id=<paste session id from step 1>"
```

**Step 3.** Search via MCP for a semantically related query.

```bash
bru run "memory/testing/mcp-search.yml" --env ascend-local --env-var "mcp_session_id=<paste session id from step 1>"
```

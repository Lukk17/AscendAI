...
| Vector store | Qdrant | Already a platform prerequisite for ascend-ai-agent's RAG pipeline. Avoids a second vector database. See [ADR-002](../decisions/ADR-002-qdrant-as-vector-backend.md). |
...
| Dual API surface | REST + MCP | REST serves ascend-ai-agent (Java HTTP client). MCP serves any FastMCP-capable agent. Both share `get_memory_client()` so there is one code path. See [ADR-004](../decisions/ADR-004-mcp-mirrors-rest-contract.md). |

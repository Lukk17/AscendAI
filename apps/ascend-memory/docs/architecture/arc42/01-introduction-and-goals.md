...
ascend-ai-agent. The MCP server at `/mcp` exposes the same four operations as FastMCP tools so any MCP-capable agent can
...
| Role | Expectation |
| :--- | :---------- |
| ascend-ai-agent (primary caller) | Fast insert and search via REST; sub-second p99 on typical user memory sets. |

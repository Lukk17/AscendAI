...
AscendMemory must be reachable both from ascend-ai-agent (which uses an HTTP client) and from any FastMCP-capable agent
...
body but awkward as a typed MCP tool argument. ascend-ai-agent uses the REST endpoint and can pass `messages`; MCP
...
- **Cons**: ascend-ai-agent is a Java Spring Boot service. It calls AscendMemory via a Java `RestClient`. Adding MCP
  client setup in Java for an internal service adds complexity. REST is already available via Spring's HTTP client.
  See ascend-ai-agent ADR-003.
- **Why not**: REST is the natural fit for ascend-ai-agent's existing Java HTTP infrastructure.

...
(ascend-ai-agent), or at a separate identity/auth layer.
...
- **Cons**: Adds an auth dependency (key management, token validation library) to a service that is only reachable
  from ascend-ai-agent on the internal compose network. Significant complexity for a service that is not directly exposed
  to the internet.
- **Why not**: Authentication is the responsibility of ascend-ai-agent, which enforces its own auth before calling
...
- **Caller misconfiguration**: ascend-ai-agent passes the `user_id` it receives from the chat request. If ascend-ai-agent

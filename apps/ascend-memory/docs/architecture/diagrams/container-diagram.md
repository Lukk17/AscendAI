...
ascend-ai-agent is the primary REST caller. It calls `/api/v1/memory/insert` after each chat turn (via
`SemanticMemoryExtractor`) and `/api/v1/memory/search` at prompt time (via `SemanticMemoryClient`). The MCP surface
at `/mcp` exposes the same operations as tools; ascend-ai-agent does not use this path, but any FastMCP-capable agent
can.

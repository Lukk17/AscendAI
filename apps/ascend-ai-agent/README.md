...
backed by Qdrant, and integrates with the dedicated [ascend-memory](../ascend-memory/) service for long-term semantic
memory.
...
```mermaid
flowchart TD
    User["User"] -- "POST /api/v1/ai/prompt" --> AscendAiAgent["ascend-ai-agent"]

    subgraph Core ["Core Orchestration"]
        AscendAiAgent -- "1. Context Retrieval" --> Redis["Redis (Active Memory)"]
        AscendAiAgent -- "2. History Lookup" --> Postgres["PostgreSQL (Long-term)"]
        AscendAiAgent -- "3. Soft-RAG (Thresholded)" --> Qdrant["Qdrant (RAG)"]
        AscendAiAgent -- "4. Memory Search & Insert" --> AscendMemory["ascend-memory"]
    end

    subgraph Providers ["AI Providers (per-request selection)"]
        AscendAiAgent -- "ChatModelResolver" --> ProviderRouter{"provider param"}
    ...
    subgraph MCP ["Model Context Protocol (MCP)"]
        AscendAiAgent -- "Tool Discovery & Calls" --> ExtTools["External Tools"]
    ...
    subgraph Ingestion ["Ingestion Pipeline"]
        ObjectStore["Object Store (S3)"] -- "Syncs Files" --> AscendAiAgent
        AscendAiAgent -- "Route: /obsidian" --> MarkdownParser["CommonMark Parser"]
        AscendAiAgent -- "Route: /documents" --> UnstructuredAPI["Unstructured API"]
...
The ascend-ai-agent uses a retrieval-gated approach.
...
- **Semantic memory (REST).** Retrieval: user-scoped memories are pulled from
  [ascend-memory](../ascend-memory/) and injected as optional context. Storage: after generating a response, the agent
...
#### 2. Run the ascend-ai-agent

The default `docker compose up -d --build` already builds and starts ascend-ai-agent as the `ascend-ai-agent` container on
...
```bash
docker compose up -d --build --force-recreate ascend-ai-agent
```

Tail the logs:

```bash
docker logs --tail 80 -f ascend-ai-agent
```

...
The ascend-ai-agent routes chat requests to multiple AI providers (lmstudio, openai, gemini, anthropic, minimax). Each provider may use a different embedding model with a different vector dimension:
...
The ascend-ai-agent's `embeddingProvider` (resolved from the request or from `AiProviderProperties.defaultEmbedding`) is forwarded as:

...
```bash
ls apps/ascend-ai-agent/e2e/fixtures/compaction-seeds/seed-compaction-idempotency.sql apps/ascend-ai-agent/e2e/fixtures/compaction-seeds/seed-compaction-idempotency.redis
```
...
```bash
docker exec -i postgres psql -U postgres -d ascend_ai < apps/ascend-ai-agent/e2e/fixtures/compaction-seeds/seed-compaction-idempotency.sql
```
...
```bash
docker cp apps/ascend-ai-agent/e2e/fixtures/compaction-seeds/seed-compaction-idempotency.redis redis:/tmp/seed-compaction-idempotency.redis
```
...
(`rm: can't remove 'C:/Users/.../Temp/seed-compaction-idempotency.redis'`), while a path embedded inside a quoted
`sh -c` string is left alone. PowerShell has no such rewriting, so the same form works unchanged there too.

```bash
docker exec redis sh -c "rm /tmp/seed-compaction-idempotency.redis"
```
...
- `apps/ascend-ai-agent/e2e/fixtures/compaction-seeds/seed-compaction-idempotency.sql`
- `apps/ascend-ai-agent/e2e/fixtures/compaction-seeds/seed-compaction-idempotency.redis`

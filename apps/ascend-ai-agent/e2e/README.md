...
```json
{
  "permissions": {
    "allow": [
      ...
      "Bash(docker exec ascend-ai-agent sh -c *)",
      "Bash(docker exec ascend-ai-agent printenv *)",
      "Bash(docker exec ascend-ai-agent ls *)",
      ...
      "Bash(docker cp apps/ascend-ai-agent/e2e/fixtures/compaction-seeds/* redis:/tmp/*)",
...
Every entry names a specific container (`postgres`, `redis`, `ascend-ai-agent`, `ascend-audio-scribe`, `ascend-ocr`,
`ascend-web-hunter`) or a specific localhost port (`:6333` Qdrant, `:9070` object store, `:9917` ascend-ai-agent, `:9998`

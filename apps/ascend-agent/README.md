...
```mermaid
flowchart TD
    User["User"] -- "POST /api/v1/ai/prompt" --> AscendAiAgent["ascend-agent"]
...
The default `docker compose up -d --build` already builds and starts ascend-ai-agent as the `ascend-agent` container on

...
The Bruno request returned HTTP 200 in 23297 ms. The response `content` field quoted all twelve canary proper nouns from the spec's Expected list. Neither refusal phrase appeared. This is the fourth of five back to back executions run to verify the retry and fan out fix for the intermittent 422 `Failed to route PDF page` error. Response time fell within the same 17 to 30 second band as the other three runs so far, so nothing in this run's timing suggests a retry fired.

Input tokens: 233 (promptTokens, from response `metadata.usage`)

Output tokens: 730 (completionTokens, from response `metadata.usage`)

Start (UTC): 2026-09-03T20:20:13Z

End (UTC): 2026-09-03T20:21:00Z
...
Captured the full Bruno JSON result with `bru run ... -o run4-result.json`, written to the session scratchpad directory rather than into `e2e/testing/runs/`, to inspect `response.data.metadata.usage` for token counts.

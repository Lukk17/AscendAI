...
- [ ] `docker exec ascend-ocr printenv MCP_FILE_URI_ROOT` returns empty output (env var unset)
...
- [ ] Send `mcp-file-uri-disabled.yml` via `bru run` with `--env-var "mcp_session_id=<captured session id>"` and wait for HTTP 200

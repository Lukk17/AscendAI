...
When `MCP_FILE_URI_ROOT=/var/lib/ascend-ocr/uploads` is set, attempt
`file:///var/lib/ascend-ocr/uploads/../../etc/passwd`. The jail rejects with the same `UNSAFE_URI` code via the

...
The first cut of the tool accepted a server-local `file_path`. That was rejected because it pushes the operator into mounting fixture directories into the container — a workflow inconsistent with how the rest of the stack moves bytes (REST multipart uploads for the agent's RAG, signed URLs for ascend-ai-agent attachments). It was also a path-traversal vulnerability: any caller able to invoke the tool could OCR `/etc/passwd`, `/run/secrets/*`, mounted volumes.
...
- **No new upload endpoint.** The REST `/v1/ocr` endpoint already accepts multipart uploads; agents that need direct upload semantics use that. `file://` exists for the case where the agent and ascend-ocr share a volume (e.g., a Kubernetes pod with an emptyDir).
...
- **No content sniffing yet.** A 200 OK with `Content-Type: image/png` and a body that is actually a zip bomb still reaches ascend-ocr. Future hardening: magic-byte sniffing via `python-magic` before invoking the OCR engine.
...
- **Drop `file://` entirely.** Rejected. There are legitimate operator workflows (shared volume between agent and ascend-ocr pods, manual debugging from a host shell into the container) where `file://` jailed to a known root is the simplest answer.

## Related

- `apps/ascend-ocr/src/api/mcp/mcp_server.py` — `ocr_process`, `_fetch_file`, `_read_jailed_file`, `_download_http`, `_validate_host`, `_is_blocked`, `_is_within`.
- `apps/ascend-ocr/src/config/config.py` — `MCP_FILE_URI_ROOT`, `MCP_ALLOWED_HOSTS`, `MCP_DOWNLOAD_TIMEOUT_SECONDS`.
- `apps/ascend-ocr/e2e/testing/6-mcp-ocr-test.md` — operator-facing test that exercises this path; requires `host.docker.internal` in `MCP_ALLOWED_HOSTS`.

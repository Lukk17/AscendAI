...
| Term | Definition |
| :--- | :--- |
| **MCP** | Model Context Protocol. An open standard for LLM tool integration. ascend-ocr uses FastMCP to expose the `ocr_process` tool via Streamable HTTP at `POST /mcp`. |
| **FastMCP** | Python library for building MCP servers on top of FastAPI / Starlette. ascend-ocr uses version 3.3.1 (`pyproject.toml`). |
| **ocr_process** | The single MCP tool exposed by ascend-ocr. Accepts `file_uri` (URI string) and `lang` (language code). Returns a serialised `OcrJsonResponse`. |
...
| **SSRF** | Server-Side Request Forgery. An attack where a server is tricked into fetching a URI that reaches internal infrastructure. ascend-ocr's guard is in `src/api/mcp/mcp_server.py:_validate_host`. |
...
| **Lifespan** | The FastAPI/Starlette async context manager that runs at startup and shutdown. ascend-ocr uses it for engine warm-up (`warm_up_engine`) and for opening/closing the MCP `aiohttp.ClientSession`. |
...
| **S3-compatible object store** | The document store used across the monorepo stack, reached over the S3 REST API. ascend-ocr's MCP tool fetches files from it when `file_uri` is an `http://host.docker.internal:9070/...` URL. Requires `MCP_ALLOWED_HOSTS=host.docker.internal` to bypass the SSRF IP block. Locally provided by a self-hosted emulator; an external prerequisite, not a compose service. |
...
| **OCR worker process** | The single-worker `ProcessPoolExecutor` started by `start_worker_pool` (`src/service/ocr_service.py`). Runs `PaddleOCR.predict()` outside the main process because the call holds the interpreter lock for its whole duration, which would otherwise stall the event loop and its `/health` and `/ready` probes. Submitted via `loop.run_in_executor(get_process_pool(), ...)`. |

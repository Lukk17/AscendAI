...
Within the monorepo, ascend-ocr is a peer of ascend-audio-scribe and ascend-web-hunter: a FastMCP-based Python service that the
ascend-ai-agent calls via the Model Context Protocol. It has no database, no message queue, and no dependencies on the
other MCP services.
...
| ascend-ai-agent | An MCP `ocr_process` tool that accepts a URI, fetches the file, and returns structured text. |

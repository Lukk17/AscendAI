...
| :--- | :--- | :--- | :--- |
| ascend-ai-agent | Inbound | MCP (Streamable HTTP) via `POST /mcp` | Primary consumer. Sends `tools/call name="ocr_process"` with a `file_uri` argument pointing to the S3-compatible object store or another HTTP source. |
| ascend-ai-agent | Inbound | HTTP multipart `POST /v1/ocr` | Secondary path for direct upload (e.g. developer tooling, curl, Bruno collection). |

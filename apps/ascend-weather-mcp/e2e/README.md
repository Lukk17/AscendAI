...
```text
apps/ascend-weather-mcp/e2e/
├── README.md                            # this file
├── fixtures/                            # canary inputs (none today; ascend-weather-mcp tools take string args, not files)
│   └── README.md
...
additionally requires an ascend-weather-mcp container restart to clear the geocoding cache between two probes.
...
flowchart LR
    ...
    Req -->|HTTP POST /mcp| Mcp[ascend-weather-mcp :9998]
...
3. **Reset state.** One command per code block, executed in order, to wipe state so the test is reproducible. Most
   ascend-weather-mcp tests do not need reset; tests that need a cold cache restart the container.
...
`<UTC-timestamp>_<N>-<feature>-tasks.md` and fills it in. ascend-weather-mcp specs make no paid call (Open-Meteo only), so
...
1. Docker compose stack up: `docker compose up -d --build ascend-weather-mcp` (or include in the full `ascend-ai` stack).
...
If the ascend-weather-mcp startup readiness banner shows any `[FAILED]` rows under `External services`, fix the upstream

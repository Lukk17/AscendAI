...
tool, `getCurrentWeather`, that ascend-ai-agent (or any MCP-compatible client) can call to fetch weather data.
...
### How it talks to ascend-ai-agent

The server runs as a normal Spring Boot web app on port `9998` and serves MCP over **SSE** (Server-Sent Events).
ascend-ai-agent's MCP client subscribes at startup and discovers the `getCurrentWeather` tool automatically.
...
```bash
docker logs --tail 80 -f ascend-weather-mcp
```

Force a clean recreate after a Dockerfile or env-var change:

```bash
docker compose up -d --build --force-recreate ascend-weather-mcp
```
...
```bash
docker run -d -p 9998:9998 --name ascend-weather-mcp ascend-weather-mcp:latest
```
...
### How ascend-ai-agent registers the tool
...
self-contained Spring Boot module that ascend-ai-agent picks up over MCP without code changes on the agent side. Drop in
another `@Tool` method and ascend-ai-agent will surface it on next restart.

For larger MCP servers (audio, web search, OCR), see [ascend-audio-scribe/](../ascend-audio-scribe/), [ascend-web-hunter/](../ascend-web-hunter/),
[ascend-ocr/](../ascend-ocr/).

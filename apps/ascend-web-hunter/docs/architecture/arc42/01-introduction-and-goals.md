...
ascend-web-hunter is a single-purpose web search and content extraction microservice in the AscendAI monorepo.
It exposes two parallel surfaces: a REST API for direct integration and an MCP server for agent-driven use via
ascend-ai-agent. Given a search query it returns ranked result URLs via SearXNG; given a URL it returns extracted
plain text using a six-strategy escalation chain that ranges from a lightweight HTTP client to a human-operated
browser session.

Within the monorepo, ascend-web-hunter is a peer of ascend-audio-scribe and ascend-ocr: a FastMCP-based Python service
called by ascend-ai-agent through the Model Context Protocol. It has no database and no other inbound callers.
...
| Role | Expectation |
| :--- | :--- |
| ascend-ai-agent | MCP tools `web_search` and `web_read` that return structured results without the agent knowing which extraction strategy ran. |

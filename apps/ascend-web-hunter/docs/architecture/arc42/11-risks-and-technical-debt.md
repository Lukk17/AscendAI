...
| **Vendored blocklist goes stale silently** | Medium | Low | `src/assets/fanboy-annoyance.txt` never updates itself; only `POST /api/v1/blocklist/refresh` or a rebuild with a re-downloaded file updates it. Nothing alerts an operator when the list has aged. `GET /api/v1/blocklist/status` exposes `age_seconds` for a monitor to check, but no monitor is wired up yet. See [ADR-008](../decisions/ADR-008-blocklist-vendored-not-fetched.md). |
...
| Item | File | Priority |
| :--- | :--- | :--- |
| No `/ready` endpoint separate from `/health` | `src/main.py:56-58` | Low — the blocklist loads at module import time, before the process can reach `/health` at all, so a reachable `/health` already implies it is loaded. A formal `/ready` endpoint would still let orchestrators probe SearXNG/FlareSolverr/Redis reachability explicitly. |

...
| In-monorepo service | `apps/ascend-web-hunter/` directory inside `AscendAI` | Shares AGENTS.md conventions, docker-compose network, and the `compose.ascend-web-hunter.yaml` external services group. Cannot be extracted without carrying those dependencies. |
...
| Constraint | Impact |
| :--- | :--- |
| No separate release cadence | Ships with the monorepo. No independent image versioning beyond `0.0.5` in `pyproject.toml`. |

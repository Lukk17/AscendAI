...
sequenceDiagram
    ...
    Controller->>Orchestrator: prompt(text, image, doc, userId, provider, model)
    Orchestrator->>Context: buildSystemMessage(userId, prompt)
    Context->>Context: RAG search + semantic memory
    Context-->>Orchestrator: enriched system message
    Orchestrator->>Context: buildUserMessage(prompt, document)
    Context-->>Orchestrator: user text
    Orchestrator->>History: loadHistory(userId)
    History-->>Orchestrator: message list
    Orchestrator->>Executor: execute(userId, system, user, history, image, provider, model)
    ...
    Executor-->>Orchestrator: AiResponse
    Orchestrator->>History: saveHistory(userId, text, response)
    Orchestrator-->>Controller: AiResponse

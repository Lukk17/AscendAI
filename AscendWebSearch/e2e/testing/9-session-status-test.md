...
```powershell
docker cp AscendWebSearch/e2e/fixtures/session-status-expired-seed.json redis:/tmp/session-status-expired-seed.json
```
...
```powershell
bru run "web-search/testing/session-status-none.yml" --env ascend-local
```

Call 2 — `example.org`, seeded with the stale fixture above (the `expired` branch).

```powershell
bru run "web-search/testing/session-status-expired.yml" --env ascend-local
```
...
```powershell
bru run "web-search/testing/session-status-active.yml" --env ascend-local
```

...
Every other cheap AscendWebSearch spec either touches no external process at all or only Redis. This one is
...
like every other AscendWebSearch spec — it makes no call to any priced LLM or embedding provider and so still
...
```powershell
bru run "web-search/testing/session-establish.yml" --env ascend-local
```

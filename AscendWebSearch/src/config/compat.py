...
class Compat(dict):
    ...
    'crawlee' (v1.3.1) expects 'browserforge' to have a 'DATA_FILES' mapping, which was removed in
    'browserforge' v1.2.4.

    This shim intercepts key lookups (filenames) and returns them as-is, preventing 'crawlee' from
    crashing when it tries to look up file paths. It uses a heuristic to differentiate between
...
def apply_compatibility_patches() -> None:
    ...
    try:
        ...
        if not hasattr(browserforge.download, "DATA_FILES"):
            ...
            browserforge.download.DATA_FILES = {  # type: ignore[attr-defined]

...
async def test_playwright_injects_storage_state_when_present():
    ...
    with (
        patch("src.reader.strategies.playwright_strategy.browser_pool.get_browser", new=AsyncMock(return_value=browser)),
        patch(
        ...
        patch("src.reader.strategies.playwright_strategy.ChallengeDetector.is_login_redirect_url", return_value=False),
    ):

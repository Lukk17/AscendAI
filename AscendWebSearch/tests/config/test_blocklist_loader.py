...
def test_cache_dir_resolution_falls_back_to_settings(monkeypatch, tmp_path):
    monkeypatch.setattr("src.config.blocklist_loader.settings.BLOCKLIST_CACHE_DIR", str(tmp_path))
    loader = BlocklistLoader()
    assert loader.cache_dir == tmp_path
    assert loader.cache_dir.exists()


def test_cache_dir_explicit_path(tmp_path):
    loader = BlocklistLoader(cache_dir=str(tmp_path))
    assert loader.cache_dir == Path(tmp_path)
    assert loader.cache_dir.exists()


def test_ensure_cache_dir_creates_when_missing(tmp_path):
    target = tmp_path / "new-assets"
    BlocklistLoader(cache_dir=str(target))
    assert target.exists()


def test_load_rules_success(tmp_path):
    loader = BlocklistLoader(cache_dir=str(tmp_path))
...
def test_download_blocklist_raises_runtime_error_on_network_fail(tmp_path):
    loader = BlocklistLoader(cache_dir=str(tmp_path))

...
def _generate_response_stripping_unsupported_json_object_format(self, messages, response_format=None, tools=None, tool_choice="auto"):
...
class AscendMemoryClient:
    def __init__(self):
        ...
        logger.info(f"Initialized Mem0 Memory Client with config: Qdrant Host={settings.QDRANT_HOST}, Embedder={settings.MEM0_EMBEDDING_MODEL}")
    ...
    def add(self, user_id: str, messages: Optional[List[Dict[str, str]]] = None, text: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
        Add memory. 
        ...
        try:
            result = {}
            if messages:
                result = self.memory.add(messages=messages, user_id=user_id, metadata=metadata, infer=settings.MEM0_INFER_MEMORY)
            elif text:
                result = self.memory.add(messages=[{"role": "user", "content": text}], user_id=user_id, metadata=metadata, infer=settings.MEM0_INFER_MEMORY)
            else:
